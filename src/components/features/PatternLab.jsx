  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-collapse">
        <thead>
          <tr className="border-b border-ink/8">
            <th className="py-2 pr-3 text-left text-[0.62rem] font-normal uppercase tracking-[0.18em] text-ink/38">n</th>
            {[1,2,3,4,5,6].map(n => <th key={n} className={`${C} font-mono text-[0.72rem] font-normal text-ink/38`}>{n}</th>)}
          </tr>
        </thead>
        <tbody>
          <tr className="border-b border-ink/6">
            <td className="py-2 pr-3 text-[0.62rem] uppercase tracking-[0.14em] text-ink/50">T(n)</td>
          ...
          <tr className={scenario.degree >= 2 ? 'border-b border-ink/6' : ''}>
            <td className="py-2 pr-3 text-[0.62rem] uppercase tracking-[0.14em] text-[#5096ff]/80">Δ¹</td>
          ...
          {scenario.degree >= 2 && (
            <tr>
              <td className="py-2 pr-3 text-[0.62rem] uppercase tracking-[0.14em] text-signal/80">Δ²</td>
      ...
      <div className="mt-4 rounded-xl border border-ink/8 bg-ink/[0.03] px-4 py-3">
        <p className="text-[0.62rem] uppercase tracking-[0.18em] text-ink/40">Diagnóstico</p>
...
const StepRow = ({ step, label, text, highlight }) => (
  <div className={`flex items-start gap-3 rounded-xl px-4 py-3 ${highlight ? 'border border-signal/25 bg-signal/8' : 'bg-ink/[0.03]'}`}>
    <span className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[0.65rem] font-bold ${highlight ? 'bg-signal text-white' : 'bg-ink/12 text-ink/50'}`}>{step}</span>
    <div>
      <p className="text-[0.68rem] uppercase tracking-[0.16em] text-ink/40">{label}</p>
...
const AlgebraicSection = () => {
  ...
  return (
    <div className="space-y-5">
      {/* scenario selector */}
      <div>
        <p className="text-[0.64rem] uppercase tracking-[0.22em] text-ink/40">Tipo de patrón</p>
      ...
      <div className="grid gap-4 sm:grid-cols-[1fr_auto]">
        ...
        <div className="flex min-w-[7rem] flex-col items-center justify-center rounded-[1.3rem] border border-ink/10 bg-white px-6 py-4">
          <p className="text-[0.62rem] uppercase tracking-[0.18em] text-ink/40">T({n})</p>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
        <LabCard dark title={`Patrón figural — ${sc.label}`}>
          <p className="mt-2 text-[0.68rem] leading-5 text-paper/45">Pulsa un término para seleccionarlo.</p>
          <div className="mt-5 flex flex-wrap items-end gap-5">
            {preview.map(k => (
              <button key={k} type="button" onClick={() => setN(k)}
                ...
                <span className="text-[0.64rem] text-paper/55">T({k}) = {sc.formula(k)}</span>
              </button>
            ))}
          </div>
          {n > 1 && (
            <div className="mt-4 rounded-xl bg-paper/10 px-4 py-3">
              <p className="text-[0.62rem] uppercase tracking-[0.16em] text-paper/45">De T({n-1}) a T({n})</p>
        ...
        <LabCard title="Tabla de diferencias">
          <p className="mt-2 text-[0.68rem] leading-5 text-ink/50">Diferencias sucesivas revelan el grado del patrón.</p>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-[1fr_auto]">
        ...
        <div className="flex min-w-[7rem] flex-col items-center justify-center rounded-[1.3rem] border border-ink/10 bg-white px-6 py-4">
          <p className="text-[0.62rem] uppercase tracking-[0.18em] text-ink/40">F({n})</p>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <LabCard dark title="Sucesión de Fibonacci">
          <p className="mt-2 text-[0.68rem] leading-5 text-paper/45">F(1)=1, F(2)=1, F(n) = F(n−1)+F(n−2)</p>
          <div className="mt-4 flex flex-wrap gap-2">
            {active.map((v,i) => (
              <div key={i} className={`flex flex-col items-center rounded-xl px-3 py-2 ${i===n-1 ? 'border border-signal/40 bg-signal/20' : 'border border-white/10 bg-white/8'}`}>
                <span className="text-[0.58rem] text-paper/40">F({i+1})</span>
          ...
          {n >= 3 && (
            <div className="mt-4 rounded-xl bg-paper/10 px-4 py-3">
              <p className="text-[0.62rem] uppercase tracking-[0.16em] text-paper/45">Regla</p>
        ...
        <LabCard title="Espiral de Fibonacci">
          <p className="mt-2 text-[0.68rem] leading-5 text-ink/50">Cuadrados de lado F(k) — la curva se aproxima a la espiral áurea.</p>
      ...
      <LabCard title="Convergencia a la razón áurea φ">
        <p className="mt-2 text-[0.68rem] leading-5 text-ink/50">La razón F(n+1)/F(n) se acerca a φ = (1+√5)/2 ≈ 1.61803…</p>
        <div className="mt-4 grid gap-5 md:grid-cols-[1.5fr_0.5fr]">
          ...
          <div className="space-y-3">
            <div className="rounded-xl border border-[#5096ff]/25 bg-[#5096ff]/8 px-4 py-3">
              <p className="text-[0.62rem] uppercase tracking-[0.16em] text-[#5096ff]/65">Razón actual</p>
            ...
            <div className="rounded-xl border border-ink/10 bg-white px-4 py-3">
              <p className="text-[0.62rem] uppercase tracking-[0.16em] text-ink/40">Error |r−φ|</p>
      ...
      <LabCard title="Tabla de diferencias — contraste con patrones polinomiales">
        <p className="mt-2 text-[0.68rem] leading-5 text-ink/50">Aquí ninguna diferencia se vuelve constante — crecimiento exponencial, no polinomial.</p>
        <div className="mt-4 overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-ink/8">
                <th className="py-2 pr-3 text-left text-[0.62rem] font-normal uppercase tracking-[0.18em] text-ink/38">n</th>
                {active.slice(0,8).map((_,i) => <th key={i} className={`${C} font-mono text-[0.72rem] font-normal text-ink/38`}>{i+1}</th>)}
              </tr>
            </thead>
            <tbody>
              <tr className="border-b border-ink/6">
                <td className="py-2 pr-3 text-[0.62rem] uppercase tracking-[0.14em] text-ink/50">F(n)</td>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2">
        {TABS.map(t => (
          ...
          >
            ...
            <p className="mt-0.5 text-[0.68rem] text-ink/45">{t.sub}</p>
          </button>
        ))}
      </div>
