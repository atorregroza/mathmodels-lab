...
const families = [
  ...
  },
  {
    ...
    generalModel: String.raw`f(x) = a(x - h)^{2} + k`,
    ...
    formula: (p) => `f(x) = ${format(p.a)}(x - ${format(p.h)})^{2} ${p.k >= 0 ? '+' : '-'} ${format(Math.abs(p.k))}`,
    domain: 'R',
  ...
  },
  {
    ...
    generalModel: String.raw`f(x) = a(x - r_{1})(x - r_{2}) \cdots (x - r_{n})`,
    parameterSet: 'a, r₁, r₂, …, rₙ ∈ ℝ',
    conditions: 'a ≠ 0, n ∈ ℕ',
  ...
  },
  {
    ...
    generalModel: String.raw`f(x) = a \cdot \ln(x - h) + c`,
    parameterSet: 'a, h, c ∈ ℝ',
    conditions: 'a ≠ 0, x > h',
